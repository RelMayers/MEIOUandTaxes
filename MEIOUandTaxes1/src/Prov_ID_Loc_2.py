...
f = open('tn_id_index.txt','r')
ids = f.readlines()
f.close()

f = open('tn_id.txt','r',encoding='utf-8-sig')
...
cust_localisation_tmpl = """
        text = {
                localisation_key = Node%id

                trigger = {
        is_variable_equal = { which = which_Node%variable value = %id }
                }
        }
"""

localisation_template = ''' Node%id: "%name"
'''
...
f = open('localisation\\DISP-Node_ID_Loc_l_english.yml','w',encoding='utf-8-sig')
f.write(localisation_buffer2)
f.close

f = open('customizable_localization\\DISP-Node_ID_Loc'+var_name+'.txt','w')
